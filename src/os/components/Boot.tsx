...
export function Boot() {
  ...
  const [username, setUsername] = useState(() => {
    try {
      return localStorage.getItem("jason-os-last-user") || "";
    } catch {
      return "";
    }
  });
  ...
  // ⭐ NEW: If OS finished loading AND user is logged in → go to desktop
  useEffect(() => {
    if (!state.loading && state.currentUserId) {
      // Your app likely uses a router or screen manager.
      // If you have a screen state, set it here.
      // Example:
      // os.setScreen("desktop");
    }
  }, [state.loading, state.currentUserId]);

  // ⭐ NEW: Show nothing until OS finishes initializing
  if (state.loading) {
    return (
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res =
      mode === "login"
        ...
        : await os.signup(username, password);

    if (!res.ok) {
      setErr(res.error || "Error");
    } else {
      try {
        localStorage.setItem("jason-os-last-user", username);
      } catch {}
    }
  }
  ...
  return (
    <div
      className="fixed inset-0 wallpaper flex items-center justify-center"
      style={{ ["--wallpaper-url" as never]: "var(--boot-wp)" }}
    >
      ...
      <div className="relative glass-strong rounded-2xl p-8 w-[360px] animate-fade-up shadow-2xl">
        <div className="text-center mb-6">
          ...
          <p className="text-xs os-text-muted mt-1">
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <button
            type="submit"
            className="w-full os-accent-bg text-white py-2 rounded-lg font-medium text-sm hover:brightness-110"
          >
